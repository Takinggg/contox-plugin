...
</html>`}function Ge(i,e){return k.commands.registerCommand("contox.init",async()=>{let t=await i.getApiKey();if(!t){k.window.showWarningMessage('Contox: Not logged in. Run "Contox: Login" first.');return}let o=k.workspace.workspaceFolders;if(!o||o.length===0){k.window.showErrorMessage("Contox: Open a workspace folder first.");return}let s=o[0].uri.fsPath,n=ze.join(s,".contox.json");if(ye.existsSync(n)&&await k.window.showWarningMessage("Contox: This workspace is already initialized. Reconfigure?","Yes","No")!=="Yes")return;let r=await k.window.showInputBox({prompt:"Enter your Contox organization (team) ID",placeHolder:"e.g. 6632a1\u2026",ignoreFocusOut:!0});if(!r)return;let a=await i.listProjects(r);if(a.error){k.window.showErrorMessage(`Contox: ${a.error}`);return}let p=[...(a.data??[]).map(y=>({label:y.name,description:`${y.contextsCount} context${y.contextsCount===1?"":"s"}`,project:y})),{label:"$(add) Create a new project...",description:""}],m=await k.window.showQuickPick(p,{placeHolder:"Select a project to link to this workspace",ignoreFocusOut:!0});if(!m)return;let d=m.project;if(!d){k.window.showInformationMessage('Create a new project on the Contox dashboard, then run "Contox: Initialize Project" again.');return}let v={teamId:r,projectId:d.id,projectName:d.name};ye.writeFileSync(n,JSON.stringify(v,null,2)+`
`);let f=k.workspace.getConfiguration("contox").get("apiUrl","https://contox.dev"),g=await e.secrets.get("contox-hmac-secret");try{await he(e),Y(t,f,r,d.id,s,g??void 0,e)}catch(y){console.error("Contox: Failed to auto-configure MCP:",y)}k.window.showInformationMessage(`Contox: Linked workspace to "${d.name}" \u2014 MCP configured for all AI tools`),await k.commands.executeCommand("contox.sync")})}var z=u(require("vscode"));var At=2,$t=3e3;function Ve(i,e,t,o){return z.commands.registerCommand("contox.sync",async s=>{let n=s?.silent===!0;if(!await i.getApiKey()){n||z.window.showWarningMessage('Contox: Not logged in. Run "Contox: Login" first.');return}let a=P();if(!a){n||z.window.showWarningMessage('Contox: No project linked. Run "Contox: Initialize Project" first.');return}t.setSyncing();let c=n?At:1,p="";for(let m=1;m<=c;m++){let d=await i.getBrain(a.projectId);if(!d.error){let v=d.data?.tree??[],f=d.data?.itemsLoaded??0;e.setTree(v,f),t.setSynced(),n?o.appendLine(`[Sync] Loaded ${String(f)} items (attempt ${String(m)})`):z.window.showInformationMessage(`Contox: Loaded ${f} memory items from "${a.projectName}"`);return}p=d.error,o.appendLine(`[Sync] Attempt ${String(m)}/${String(c)} failed: ${p}`),m<c&&await new Promise(v=>{setTimeout(v,$t)})}t.setError(),n?o.appendLine(`[Sync] Gave up after ${String(c)} attempts.`):z.window.showErrorMessage(`Contox sync failed: ${p}`)})}var $=u(require("vscode"));function qe(i,e,t){return $.commands.registerCommand("contox.create",async()=>{if(!await i.getApiKey()){$.window.showWarningMessage('Contox: Not logged in. Run "Contox: Login" first.');return}let s=P();if(!s){$.window.showWarningMessage('Contox: No project linked. Run "Contox: Initialize Project" first.');return}let n=await $.window.showInputBox({prompt:"Context name",placeHolder:"e.g. API Documentation",ignoreFocusOut:!0});if(!n)return;let r=await $.window.showInputBox({prompt:"Description (optional)",placeHolder:"e.g. REST API docs for the backend",ignoreFocusOut:!0}),a=await i.createContext(n,s.teamId,s.projectId,r||void 0);if(a.error){$.window.showErrorMessage(`Contox: Failed to create context \u2014 ${a.error}`);return}$.window.showInformationMessage(`Contox: Created context "${n}"`),t.setSyncing();let c=await i.getBrain(s.projectId);!c.error&&c.data&&e.setTree(c.data.tree,c.data.itemsLoaded),t.setSynced()})}var G=u(require("vscode")),we=u(require("fs")),Je=u(require("path"));function Ye(i){return G.commands.registerCommand("contox.reset",async()=>{if(await G.window.showWarningMessage("Contox: This will log you out and remove the workspace configuration. Continue?",{modal:!0},"Reset")!=="Reset")return;await i.clearApiKey();let t=G.workspace.workspaceFolders;if(t&&t.length>0){let o=Je.join(t[0].uri.fsPath,".contox.json");try{we.existsSync(o)&&we.unlinkSync(o)}catch{}}G.window.showInformationMessage('Contox: Reset complete. Run "Contox: Setup Wizard" to reconfigure.')})}var D=u(require("vscode")),I=u(require("fs")),X=u(require("path"));var R=u(require("fs")),Q=u(require("path")),Qe="<!-- contox:start -->",Se="<!-- contox:end -->",Dt=`# Contox \u2014 Project Memory
...
- Categories: architecture, conventions, implementation, decisions, bugs, todo`,_t=[{name:"Cursor",relPath:".cursorrules",dirHint:".cursor"},{name:"Copilot",relPath:Q.join(".github","copilot-instructions.md"),dirHint:".github"},{name:"Windsurf",relPath:".windsurfrules"},{name:"Cline",relPath:".clinerules"}];function Bt(i,e){let t=`${Qe}
${e}
